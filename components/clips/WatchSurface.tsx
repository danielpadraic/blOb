...
import { subscribeVisualViewportBox, visualViewportBox } from '@/lib/visualViewport';

export function useWatchSurface(): { style: ViewStyle; height: number } {
  const [box, setBox] = useState<VisualViewportBox>(() =>
    Platform.OS === 'web' ? visualViewportBox() : { top: 0, left: 0, width: 0, height: 0 },
  );
  ...
  useEffect(() => {
    ...
    return subscribeVisualViewportBox(setBox);
  }, []);
