...
export function UserSearchResult({ profile, relation, busy, onPrimary }: UserSearchResultProps) {
  ...
  const action = official
    ? { title: copy('official.friends'), variant: 'outline' as const, locked: true }
    : relation === 'self'
      ? null
      : { ...ACTION[relation], locked: false };
      ...
      <View className="flex-row items-center">
        ...
        <View className="ml-3 min-w-0 flex-1">
          ...
          <AppText className="mt-0.5 text-[11px] font-semibold" style={{ color: THEME.accent }}>
            {official ? copy('official.friends') : statusLabel(relation)}
          </AppText>
        </View>
        {action ? (
          <Button
            ...
            onPress={action.locked ? undefined : onPrimary}
          />
        ) : null}
      </View>
    </Card>
