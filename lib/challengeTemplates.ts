...
export const CREATE_STEP_FIELDS: Record<number, readonly (keyof CreateChallengeValues)[]> = {
  ...
  3: ['challenge_type'],
  4: ['duration_type', 'duration_days', 'starts_at', 'ends_at', 'end_mode', 'duration_value', 'duration_unit', 'frequency', 'target_count', 'join_until_preset', 'join_until_at'],
  ...
  8: ['buy_in', 'currency', 'participant_cap', 'max_participants', 'creator_participating', 'min_participants', 'misses_allowed', 'proof_review', 'host_rigor'],
...
export function previewFromValues(values: CreateChallengeValues): ChallengeWithStats {
  ...
  return {
    ...
    join_until_at: values.join_until_at || null,
    host_rigor: values.host_rigor ?? 'normal',
