...
import { isLobbyActiveParticipantStatus, isLobbyEndedChallenge } from '@/lib/lobbyChallenge';
...
const LOBBY_SELECTS = [
  ...
  'id, title, description, rules, is_official, created_by, buy_in_amount, days_required, min_minutes, proof_requirements, status, starts_at, ends_at, timezone, prize_pool, category, challenge_type, visibility, frequency, target_count, tasks, task, created_at, updated_at, sponsor_name, sponsor_logo_url',
  'id, title, description, rules, is_official, created_by, buy_in_amount, days_required, min_minutes, proof_requirements, status, starts_at, ends_at, timezone, prize_pool, category, challenge_type, visibility, task, created_at, updated_at',
  'id, title, description, rules, is_official, created_by, buy_in_amount, days_required, min_minutes, status, starts_at, ends_at, timezone, prize_pool, category, challenge_type, visibility, task',
...
export function normalizeChallenge(row: ChallengeRow): Challenge {
  ...
  const daysRequired = Number(row.days_required ?? row.length_value ?? 0) || 0;
...
export async function fetchCompetingChallenges(userId?: string): Promise<Challenge[]> {
  ...
  return sortMyLobby(
    mine.filter(
      (row) => joinedIds.has(row.id) && isLiveOrUpcoming(row.status) && !isLobbyEndedChallenge(row),
    ),
  );
}

/** Ended / settling / settled / cancelled from Official + Active + Hosting universes. */
export async function fetchEndedLobbyChallenges(userId?: string): Promise<Challenge[]> {
  ...
  const clockCreatedPromise = selectChallengeList(
    (query) =>
      query
        .eq('created_by', userId)
  ...
  for (const row of [
    ...
    ...(await joinedPromise),
    ...(await clockCreatedPromise),
    ...(await clockJoinedPromise),
  ]) {
...
export type OfficialChallengeDetailsPayload = {
  ...
  proof_requirements: Array<{ type: string; required: true }>;
  proof_type: string;
};

export async function updateOfficialChallengeDetails(
  challengeId: string,
  payload: OfficialChallengeDetailsPayload,
): Promise<Challenge> {
  const { data, error } = await supabase.rpc('update_official_challenge_details', {
    p_challenge_id: challengeId,
    p_payload: payload,
  });
  if (error) {
