...
const COUNTER_COLUMNS = 'id, user_id, title, status, parent_id, card_url, created_at, updated_at, saved_at';
...
export async function fetchLiveCounters(): Promise<CounterSummary[]> {
  ...
  return (data ?? []).map((row) => {
    const draft = draftFromRows(row, metricsFromEmbedded((row as { counter_metrics?: unknown }).counter_metrics));
    return {
      id: draft.id,
      title: draft.title,
      status: 'live',
