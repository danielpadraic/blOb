#!/usr/bin/env python3
"""Render official bob-wave onto cream PWA icon sizes."""
...
SRC = Path(__file__).resolve().parents[2] / "assets" / "mascot" / "bob-wave.png"
...
def render(size: int, rows: list[bytearray], sw: int, sh: int) -> bytearray:
    inset = int(size * PAD)
    box = size - inset * 2
...
def main() -> None:
    ...
    print(f"source {SRC.name} {width}x{height} a00={rows[0][3]}")
    OUT.mkdir(parents=True, exist_ok=True)
    for name, size in SIZES.items():
        dest = OUT / name
        write_png(dest, size, size, render(size, rows, width, height))
