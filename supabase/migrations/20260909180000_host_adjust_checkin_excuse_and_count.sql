-- Fix host_adjust_checkin: excuse must not read unassigned v_win;
-- Count must update a started check-in; Live lines must not collide
-- on posts_system_kind_uidx.
...
comment on column public.posts.system_key is
  'Per-event key for system posts that can repeat (host_counted_day = userId:periodStart). Null for singleton kinds.';

drop index if exists public.posts_system_kind_uidx;
...
create unique index if not exists posts_system_kind_uidx
  ...
  where system_kind is not null
    and system_kind is distinct from 'host_counted_day';

create unique index if not exists posts_host_counted_day_uidx
  on public.posts (author_id, challenge_id, system_kind, system_key)
  where system_kind = 'host_counted_day';
...
begin
  ...
  if v_action = 'count_honor' then
    ...
    insert into public.posts (
      ...
      source, system_kind, system_key, hidden_from_home
    ) values (
      auth.uid(),
      p_challenge_id,
      'Host counted Day ' || v_day_n::text || ' for ' || v_name || '.',
      '{}',
      'public',
      '{}',
      'challenge',
      'host_counted_day',
      p_user_id::text || ':' || v_win_start::text,
      true
    );
...
where schemaname = 'public'
  and indexname in ('posts_system_kind_uidx', 'posts_host_counted_day_uidx')
order by 1;
