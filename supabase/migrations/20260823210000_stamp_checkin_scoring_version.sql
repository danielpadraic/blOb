-- Stamp challenges.scoring_version onto new challenge_checkins rows.
-- Existing check-in rows stay null. No backfill.
...
comment on column public.challenges.scoring_version is
  'Incremented when a versioned scoring method (e.g. comparable_points) is edited live.';
comment on column public.challenge_checkins.scoring_version is
  'Challenge scoring_version at insert time. Historical scores keep this snapshot.';

create or replace function public.challenge_checkins_stamp_scoring_version()
returns trigger
language plpgsql
as $$
declare
  v_version integer;
begin
  if new.scoring_version is not null then
    return new;
  end if;
  select coalesce(scoring_version, 1)
    into v_version
    from public.challenges
    where id = new.challenge_id;
  new.scoring_version := coalesce(v_version, 1);
  return new;
end;
$$;

drop trigger if exists challenge_checkins_stamp_scoring_version on public.challenge_checkins;
create trigger challenge_checkins_stamp_scoring_version
  before insert on public.challenge_checkins
  for each row
  execute function public.challenge_checkins_stamp_scoring_version();
