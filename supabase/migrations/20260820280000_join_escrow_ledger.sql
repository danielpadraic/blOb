-- Pay entry: existing join/escrow path. wallet_ledger uses real columns only
-- (entry_type, reason, reference_id, challenge_id — never ref_type).
-- User-created stays joinable until live (min + start). Do not close because
-- starts_at is near.
...
begin
  ...
  if v_c.series_id is not null then
    ...
    -- User-created: join until live. in_progress stays open. Ignore starts_at.
    if v_c.status in (
